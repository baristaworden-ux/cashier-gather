...
export async function POST(req: NextRequest) {
  ...
  const { name, bank, account_number, account_type, currency } = await req.json()
  ...
  const { data, error } = await supabase
    .from('admin_accounts')
    .insert({ name, bank, account_number: account_number || null, account_type: account_type || 'account', currency: currency || null, color, user_id: user.id })
    .select()
    .single()
